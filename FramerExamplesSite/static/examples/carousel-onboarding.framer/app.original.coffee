...
swipeHintAnimator = ->
	if splashHintRun
		...
		splashHintAnimation = swipeHint.animate 
			...
			curve: 'linear'
			time: 0.8 
		splashHintScaleAnimation.on 'end', ->
			if splashHintRun
				Utils.delay 0.05, ->
					splashHintAnimation.start()
			else
				return
		splashHintAnimation.on 'end', ->
			if splashHintRun
				swipeHint.y = swipeHint.originalFrame.y
				swipeHint.scale = 1.3  	
				Utils.delay 1, ->
					splashHintScaleAnimation.start()
			else
				return 
swipeHintAnimator() 
...
scrollHintAnimator = ->
	if scrollHintRun
		...
		scrollHintAnimation = scrollHint.animate 
			...
			curve: 'ease-out'
			time: 0.8 
		scrollHintScale.on 'end', ->
			if scrollHintRun
				Utils.delay 0.05, ->
					scrollHintAnimation.start()
			else
				return
		scrollHintAnimation.on 'end', ->
			if scrollHintRun
				scrollHint.y = scrollHint.originalFrame.y
				scrollHint.scale = 1.3  	
				Utils.delay 1, ->
					scrollHintScale.start()
			else 
				return
		scrollHintScale.start()
